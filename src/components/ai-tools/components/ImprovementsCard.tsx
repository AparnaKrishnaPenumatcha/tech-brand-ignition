...
const ImprovementsCard: React.FC<ImprovementsCardProps> = ({ improvements }) => {
  ...
  if (!parsedImprovements || parsedImprovements.length === 0) {
    ...
    return (
      <Card className="border-l-4 border-l-gray-400 shadow-lg">
        <CardHeader className="bg-gradient-to-r from-gray-50 to-gray-100">
          <CardTitle className="text-gray-700 flex items-center gap-3">
            <div className="p-2 bg-gray-100 rounded-lg">
              <AlertTriangle className="w-6 h-6 text-gray-600" />
            </div>
            <div>
              <h3 className="text-xl font-bold">No Improvements Found</h3>
        ...
        <CardContent className="p-6">
          <p className="text-gray-600">Please try analyzing your resume again.</p>
        </CardContent>
      </Card>
    );
  }
